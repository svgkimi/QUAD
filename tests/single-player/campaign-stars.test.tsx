...
describe("performance stars", () => {
  ...
  it.each(STAGES.filter(s => s.kind === "survival"))("survival $id rewards low danger exposure, not just the final stack", stage => {
    const [two, three] = starThresholds(stage);
    for (const [dangerMs, stars] of [[0,3],[three,3],[three+1,2],[two,2],[two+1,1]]) {
      expect(earnedStageStars(stage, finished(stage, dangerMs))).toBe(stars);
    }
  });
  ...
  it("shows only stage number, progress and time in the playing HUD", async () => {
    ...
    expect(host.textContent).toBe("4 / 502 / 5줄");
